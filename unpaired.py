...
##conda activate metagenomics ##python /mnt/f/binning/binning unpaired beta1.0.py  
...
def main():
    log_file = "/mnt/e/zby/binning/pipeline.log"
    input_file = "/mnt/e/zby/binning/BK1.scaftigs.fastq"
    input_files = ["/mnt/e/zby/binning/BK1.scaftigs.fastq", "/mnt/e/zby/binning/BK2.scaftigs.fastq"]  # Example list of input files
    input_files_str = ' '.join(input_files)
    filtered_fastq = "/mnt/e/zby/binning/BK1/filtered.fastq"
    reference_genome = "/mnt/e/zby/binning/Ovis_aries_rambouillet.ARS-UI_Ramb_v2.0.dna_sm.toplevel.fa"
    cleaned_fastq = "/mnt/e/zby/binning/BK1/cleaned.fastq"
    megahit_output = "/mnt/e/zby/binning/BK1/megahit_output"
    quast_output = "/mnt/e/zby/binning/BK1/quast_output"
    coverage_file = "/mnt/e/zby/binning/BK1/coverage.tsv"
    binning_output = "/mnt/e/zby/binning/BK1/binning"
    contig_index_prefix = "/mnt/e/zby/binning/BK1/megahit_output/final.contigs"

    ## Create directories if not exist
    os.makedirs(os.path.dirname(filtered_fastq), exist_ok=True)
    os.makedirs(os.path.dirname(cleaned_fastq), exist_ok=True)
    os.makedirs(megahit_output, exist_ok=True)
    os.makedirs(quast_output, exist_ok=True)
    os.makedirs(binning_output, exist_ok=True)

    # # Step 1: Quality filtering (Single seq)
    run_command(f"fastp -i {input_files_str} -o {filtered_fastq}", log_file)

    # # Step 1: Quality filtering for unpaired reads
    #run_command(f"cat {input_files_str} > /mnt/e/zby/binning/BK1/combined_input.fastq", log_file)
    #run_command(f"fastp -i /mnt/e/zby/binning/BK1/combined_input.fastq -o {filtered_fastq}", log_file)

    # # Step 2: Remove host, food, and human sequences
    # run_command(f"bowtie2-build {reference_genome} reference_index", log_file)
    # run_command(f"bowtie2 -x reference_index -U {filtered_fastq} -S aligned.sam", log_file)
    # run_command(f"samtools view -bS aligned.sam > aligned.bam", log_file)
    # run_command(f"samtools sort aligned.bam -o sorted.bam", log_file)
    # run_command(f"samtools index sorted.bam", log_file)
    # run_command(f"bedtools bamtofastq -i sorted.bam -fq {cleaned_fastq}", log_file)

    # # Step 3: Assembly of high-quality reads
    # run_command(f"megahit -r {cleaned_fastq} -o {megahit_output}", log_file)
    # run_command(f"quast {megahit_output}/final.contigs.fa -o {quast_output}", log_file)
    ...
    # # Starting from Create Bowtie 2 index for contigs
    cleaned_fastq = "/mnt/e/zby/binning/BK1/cleaned.fastq"
    megahit_output = "/mnt/e/zby/binning/BK1/megahit_output"
    quast_output = "/mnt/e/zby/binning/BK1/quast_output"
    coverage_file = "/mnt/e/zby/binning/BK1/coverage.tsv"
    ...
    #run_command(f"bowtie2 -x {contig_index_prefix} -U {cleaned_fastq} -S {binning_output}/contig_aligned.sam", log_file)
    #run_command(f"samtools view -bS {binning_output}/contig_aligned.sam > {binning_output}/contig_aligned.bam", log_file)
    #run_command(f"samtools sort {binning_output}/contig_aligned.bam -o {binning_output}/contig_aligned_sorted.bam", log_file)
    #run_command(f"samtools index {binning_output}/contig_aligned_sorted.bam", log_file)
    #run_command(f"samtools idxstats {binning_output}/contig_aligned_sorted.bam > {coverage_file}", log_file)
    
    # 确保目录存在
    os.makedirs(binning_output, exist_ok=True)
    
    # 读取并处理 coverage 文件
    coverage_file = "/mnt/e/zby/binning/BK1/coverage_checked.tsv"
    ...
    # 保存处理后的文件
    coverage_file_checked_corrected = "/mnt/e/zby/binning/BK1/coverage_checked_corrected.tsv"
    coverage_df.to_csv(coverage_file_checked_corrected, sep='\t', index=False)
    
    # Step 6: Binning
    #run_command(f"/root/miniconda3/bin/run_MaxBin.pl -contig {megahit_output}/final.contigs.fa -out {binning_output}/maxbin2_bin -abund {coverage_file_checked_corrected}", log_file)
    #run_command(f"metabat2 -i {megahit_output}/final.contigs.fa -o {binning_output}/metabat2_bins", log_file)   
    #run_command(f"concoct --composition_file {megahit_output}/final.contigs.fa --coverage_file {coverage_file_checked_corrected} -b {binning_output}/concoct_bins --threads 4", log_file)
    ...
    #run_command(f"DAS_Tool -i {das_tool_input_file} -c {combined_contigs_file} -o {das_tool_output_dir} --write_bins", log_file)
...
#当电脑的运行内存小于40g的时候，使用 --reduced_tree模式
    ...
    #run_command(f"checkm lineage_wf --reduced_tree --nt -t 8 -x fa {extracted_bins_dir} {checkm_output_dir}", log_file)
